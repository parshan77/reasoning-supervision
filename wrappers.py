...
MedQA_SPLIT_FILE = {
  "train": "processed_med_qa_train.json",
}
...
class LoggingWrapper(gym.Wrapper):
  def __init__(self, env, folder="trajs", file_id=None):
    super().__init__(env)
    self.trajs = []
    self.traj = {"observations": [], "thoughts": [], "actions": []}
    self.folder = folder
  ...
  def reset(self, seed=None, return_info=False, options=None, idx=None):
    ...
    self.traj = {"observations": [observation], "thoughts": [], "actions": []}
    return output
...
class MedQAWrapper(gym.Wrapper):
  def __init__(self, env, split):
    super().__init__(env)
    data_file = f"{DATA_DIR}/{MedQA_SPLIT_FILE[split]}"
    self.data = json.load(open(data_file))
    self.data = [(d['question'], d['answer']) for d in self.data]
    self.data_idx = 0
    self.split = split
